...
pub fn update_tray_icon_with_theme<R: Runtime>(
    _app: &AppHandle<R>,
    theme: &str,
) -> tauri::Result<()> {
...
pub fn update_tray_icon(app: &AppHandle, mut mem_percent: u8) {
    ...
    if mem_percent == 0 {
        if let Ok(mem) = state.engine.memory() {
            ...
            tracing::info!(
                "Tray icon 0% detected, fetched real value: {}%",
                mem_percent
            );
        } else {
        ...
        if let Ok(cache) = ICON_CACHE.lock() {
            if let Some(ref cached) = *cache {
                    ...
                    && cached.transparent == tray_cfg.transparent_bg
                {
                    ...
                    tracing::debug!(
                        "Tray icon cache hit at {}%, skipping set_tray_icon",
                        mem_percent
                    );
                    return;
                }
