...
pub fn create_elevated_task() -> Result<()> {
    ...
    cmd.args([
        ...
        "once",
        "/st",
        "00:00", // Start time in the past: on-demand only, never self-fires
        "/rl",
        "highest",
        "/f",
...
pub fn run_via_elevated_task() -> Result<bool> {
    ...
    let mut cmd = Command::new("schtasks");
    cmd.args(["/run", "/tn", ELEVATED_TASK_NAME]);
...
pub fn delete_elevated_task() -> Result<()> {
    let mut cmd = Command::new("schtasks");
    cmd.args(["/delete", "/tn", ELEVATED_TASK_NAME, "/f"]);
...
pub fn elevated_task_matches_current_exe() -> bool {
    ...
    let mut cmd = Command::new("schtasks");
    cmd.args(["/query", "/tn", ELEVATED_TASK_NAME, "/xml"]);
