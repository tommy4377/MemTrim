...
fn safe_device_io_control(
    ...
    if err == ERROR_NOT_SUPPORTED || err == ERROR_INVALID_FUNCTION || err == ERROR_INVALID_PARAMETER
    {
        tracing::debug!(
...
pub fn flush_modified_file_cache_all() -> Result<()> {
    ...
    for letter in drives {
        if let Some((raw_h, access)) = open_volume(letter) {
            let h = scopeguard::guard(raw_h, |h| unsafe {
                CloseHandle(h);
            });
            volumes_total += 1;
            ...
            if privileges_acquired && access != 0 {
                ...
                let lock_ok =
                    safe_device_io_control(*h, FSCTL_LOCK_VOLUME, "FSCTL_LOCK_VOLUME", letter);

                if lock_ok {
                    safe_device_io_control(*h, FSCTL_UNLOCK_VOLUME, "FSCTL_UNLOCK_VOLUME", letter);
