...
use std::{ffi::CString, mem, ptr, sync::OnceLock};
use windows_sys::Win32::System::ProcessStatus::K32EmptyWorkingSet as PsapiEmptyWorkingSet;
use windows_sys::Win32::{
    Foundation::{CloseHandle, HANDLE},
    Security::{
        DuplicateTokenEx, RevertToSelf, SecurityImpersonation, TokenPrimary, TOKEN_DUPLICATE,
...
pub fn empty_working_set_stealth(exclusions: &[String]) -> Result<()> {
    ...
    let resolver = SyscallResolver::new().context("Failed to initialize syscall resolver")?;
    ...
    for (pid, name) in processes {
        ...
        unsafe {
            ...
            if raw_handle != std::ptr::null_mut() {
                let handle = scopeguard::guard(raw_handle, |h| {
                    CloseHandle(h);
                });
                ...
                match execute_indirect_syscall_empty_working_set(ssn, *handle) {
                    Ok(status) if status == 0 => {
                        tracing::debug!(
                            "✓ Stealth EmptyWorkingSet successful for PID {} (indirect syscall)",
                            pid
                        );
                    }
                    Ok(status) => {
                        tracing::debug!(
                            "Indirect syscall failed for PID {} (0x{:08X}), trying direct",
                            pid,
                            status as u32
                        );
                        ...
                        if direct_status != 0 {
                            tracing::debug!(
                                "Direct syscall also failed for PID {} (0x{:08X})",
                                pid,
                                direct_status as u32
                            );
                        }
                    }
                    Err(e) => {
                        tracing::debug!(
                            "Indirect syscall error for PID {}: {}, falling back",
                            pid,
                            e
                        );
...
unsafe fn execute_indirect_syscall_empty_working_set(
    ssn: u32,
    ...
    let syscall_ptr =
        syscall_addr.ok_or_else(|| anyhow::anyhow!("Could not find syscall instruction"))?;

    tracing::debug!(
        "Using indirect syscall for EmptyWorkingSet at address: 0x{:x}",
...
unsafe fn execute_direct_syscall_empty_working_set(
    ssn: u32,
    process_handle: windows_sys::Win32::Foundation::HANDLE,
) -> i32 {
    let mut status: i32;
    ...
    fn new() -> Result<Self> {
        unsafe {
            ...
            if e_lfanew == 0 || e_lfanew > MAX_E_LFANEW {
                bail!(
                    "e_lfanew offset {} is out of valid range (0..{})",
                    e_lfanew,
                    MAX_E_LFANEW
                );
            }
            ...
            if pe_signature != PE_SIGNATURE {
                bail!(
                    "Invalid PE signature at offset 0x{:X} (expected 0x{:08X}, got 0x{:08X})",
                    e_lfanew,
                    PE_SIGNATURE,
                    pe_signature
                );
            ...
            let size = match magic {
                PE32PLUS_MAGIC => {
                    ...
                    let size_of_image = ptr::read_unaligned(ptr::addr_of!(
                        (*nt_header).optional_header.size_of_image
                    ));
                    tracing::debug!(
                        "Detected PE32+ (64-bit) ntdll, size_of_image = 0x{:X}",
                        size_of_image
                    );
            ...
            if size < 4096 {
                bail!(
                    "size_of_image ({}) is smaller than one memory page — corrupted PE header",
                    size
                );
            }
            if size > 256 * 1024 * 1024 {
                bail!(
                    "size_of_image (0x{:X}) exceeds 256 MB — likely corrupted PE header",
                    size
                );
            }
...
unsafe fn try_system_token_duplication() -> Result<()> {
    match std::panic::catch_unwind(|| {
        ...
        let h_process = scopeguard::guard(h_process, |h| {
            CloseHandle(h);
        });
        ...
        let h_token = scopeguard::guard(h_token, |h| {
            CloseHandle(h);
        });
        ...
        let h_new_token = scopeguard::guard(h_new_token, |h| {
            CloseHandle(h);
        });
...
unsafe fn execute_indirect_syscall(
    ...
    tracing::debug!(
        "Using indirect syscall at address: 0x{:x}",
        syscall_ptr as usize
    );
...
unsafe fn execute_nt_set_system_info(info_class: u32, command: u32) -> i32 {
    let mut cmd = command;
...
pub fn trim_memory_compression_store() -> Result<()> {
    ...
    unsafe {
        ...
        let _guard = impersonate_system_token().context("Failed to acquire SYSTEM privileges")?;
...
unsafe fn try_trim_with_resolver() -> Result<()> {
    ...
    let ssn = resolver
        .get_ssn("NtSetSystemInformation")
    ...
    if status == 0 {
        ...
        tracing::warn!(
            "Direct syscall failed (0x{:08X}), trying indirect syscall...",
            status as u32
        );
        ...
        match execute_indirect_syscall(
            ...
            Ok(indirect_status) if indirect_status == 0 => {
                tracing::info!(
                    "✓ Memory Compression Store trimmed successfully (indirect syscall)"
                );
                Ok(())
            }
            Ok(indirect_status) => {
                tracing::warn!(
                    "Indirect syscall also failed (0x{:08X})",
                    indirect_status as u32
                );
...
unsafe fn try_trim_direct_nt() -> Result<()> {
    ...
    for cmd in commands {
        ...
        if status == 0 {
            tracing::info!(
                "✓ Memory Compression Store trimmed via direct NT call (cmd={})",
                cmd
            );
            return Ok(());
            ...
            tracing::warn!(
                "Direct NT call returned NTSTATUS: 0x{:08X} (cmd={})",
                status as u32,
                cmd
            );
        }
...
pub fn purge_standby_list() -> Result<()> {
    ...
    unsafe {
        ...
        let _guard = match impersonate_system_token() {
            ...
            Err(e) => {
                tracing::warn!(
                    "Could not acquire SYSTEM privileges: {}. Using standard API.",
                    e
                );
...
pub fn aggressive_modified_page_flush_stealth() -> Result<()> {
    ...
    match unsafe {
        ...
        Ok(status) => {
            tracing::warn!(
                "Indirect syscall failed (0x{:08X}), trying direct syscall",
                status as u32
            );
        ...
        Err(e) => {
            tracing::warn!(
                "Indirect syscall error: {}, falling back to direct syscall",
                e
            );
...
unsafe fn try_modified_direct_nt() -> Result<()> {
    ...
    for cmd in commands {
        ...
        if status == 0 {
            tracing::info!(
                "✓ Modified page list flushed via direct NT call (cmd={})",
                cmd
            );
            return Ok(());
        } else if status as u32 != 0xC000000D {
...
unsafe fn try_registry_with_resolver() -> Result<()> {
    ...
    if status == 0 {
        ...
        tracing::warn!(
            "Advanced registry optimization returned NTSTATUS: 0x{:08X}",
            status as u32
        );
        Err(anyhow::anyhow!("Advanced approach failed"))
    }
