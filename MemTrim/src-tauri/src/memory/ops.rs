        ...
        if low_priority {
            if use_stealth {
                ...
                match crate::memory::advanced::purge_standby_list_low_priority_stealth() {
                    Ok(_) => {
                        tracing::info!(
                            "✓ Advanced low priority standby list purge successful (stealth mode)"
                        );
                        Ok(())
                    }
                    Err(e) => {
                        tracing::warn!(
                            "⚠ Advanced low priority standby purge failed ({}), using standard API",
                            e
                        );
            ...
            if use_stealth {
                ...
                match crate::memory::advanced::purge_standby_list_stealth() {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            "⚠ Advanced standby purge failed ({}), using standard API",
                            e
                        );
                        let result =
                            nt_call_u32(SYS_MEMORY_LIST_INFORMATION, MEM_EMPTY_WORKING_SETS + 1);
                        if result.is_ok() {
    ...
    crate::antivirus::whitelist::safe_memory_operation(|| {
        if use_stealth {
            ...
            match crate::memory::advanced::aggressive_modified_page_flush_stealth() {
                ...
                Err(e) => {
                    tracing::warn!(
                        "⚠ Advanced modified page flush failed ({}), using standard API",
                        e
                    );
...
pub fn optimize_registry_cache() -> Result<()> {
    ...
    crate::antivirus::whitelist::safe_memory_operation(|| {
        ...
        match crate::memory::advanced::optimize_registry_cache() {
            ...
            Err(e) => {
                tracing::warn!(
                    "⚠ Advanced registry optimization failed ({}), using standard API",
                    e
                );
                unsafe {
...
fn empty_ws_process(pid: u32) -> bool {
    ...
    for attempt in 1..=MAX_RETRIES {
        unsafe {
            ...
            let h = scopeguard::guard(raw_h, |h| {
                CloseHandle(h);
            });
    ...
    crate::antivirus::whitelist::safe_memory_operation(|| {
        if use_stealth {
            ...
            match crate::memory::advanced::empty_working_set_stealth(exclusions) {
                ...
                Err(e) => {
                    tracing::warn!(
                        "⚠ Stealth Working Set optimization failed ({}), using standard API",
                        e
                    );
