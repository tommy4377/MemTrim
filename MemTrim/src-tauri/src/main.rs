...
fn reacquire_single_instance_mutex() {
    ...
    if !handle.is_null() {
        if unsafe { GetLastError() } == ERROR_ALREADY_EXISTS as u32 {
            ...
            tracing::warn!(
                "Mutex already exists while reacquiring - an elevated instance may be running"
            );
        }
...
fn launch_elevated_instance() -> Result<(), String> {
    ...
    let dir_wide = current_exe.parent().map(|d| to_wide(&d.to_string_lossy()));
...
async fn perform_optimization(
    ...
    tracing::info!(
        "Notification check: show_settings={}, reason={:?}",
        show_notif,
        reason
    );
        ...
        if let (Ok(res), Some(aft)) = (result, after) {
            ...
            if freed_mb > 1.0 || has_successful_area {
                ...
                let title_key = match reason {
                    Reason::Manual => "MemTrim • Optimization completed",
                    Reason::Schedule => "MemTrim • Scheduled optimization",
                    Reason::LowMemory => "MemTrim • Low memory optimization",
                    Reason::Hotkey => "MemTrim • Hotkey optimization",
                };
                ...
                tracing::debug!(
                    "Emitted optimization-completed event with {} MB freed, result: {:?}",
                    freed_mb.abs(),
                    event_result
                );
...
fn main() {
    ...
    #[cfg(windows)]
    {
        use crate::system::{
            elevated_task::{
                create_elevated_task, elevated_task_matches_current_exe, run_via_elevated_task,
            },
            is_app_elevated,
        };
                ...
                if let Ok(config) = serde_json::from_str::<crate::config::Config>(&config_str) {
                    if config.request_elevation_on_startup {
                        ...
                        if !is_elevated {
                            ...
                            tracing::info!(
                                "Elevating via UAC prompt (scheduled task unavailable)..."
                            );
                            close_single_instance_mutex();
                            match launch_elevated_instance() {
                                Ok(()) => {
        ...
        if is_elevated {
            ...
            tracing::warn!(
                "Application running without elevated privileges - some features may be limited"
            );
