...
pub fn cmd_check_elevation() -> Result<serde_json::Value, String> {
    #[cfg(target_os = "windows")]
    {
        ...
        let has_debug_priv =
            crate::memory::privileges::ensure_privilege("SeDebugPrivilege").is_ok();
        let has_quota_priv =
            crate::memory::privileges::ensure_privilege("SeIncreaseQuotaPrivilege").is_ok();
        let has_profile_priv =
            crate::memory::privileges::ensure_privilege("SeProfileSingleProcessPrivilege").is_ok();
