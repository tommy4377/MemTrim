...
export async function initApp(): Promise<void> {
  // One-time UI storage migration from the pre-MemTrim key.
  const legacyTheme = localStorage.getItem('tmc_theme')
  if (localStorage.getItem('memtrim_theme') === null && legacyTheme !== null) {
    localStorage.setItem('memtrim_theme', legacyTheme)
  }
